...
const playingObject = {
  ...
  updateFrames() {
    if (player.gameActive) {
      this.frames += screenFrame % 10 == 0 ? 1 : 0; 
      this.position.y += screenFrame % 8 == 0 ? Math.sin(screenFrame * RAD) : 0; 
    ...
    this.frames = this.frames % this.imgAnimation.length; 
  },
  flying() {
    
    if (this.position.y > 0) {
      SFX.flap.play();
      this.speed = -this.thrust%10;
    }
  },
  objectRotation() {
    if (this.speed <= 0) {
